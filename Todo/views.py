...
def index(request):
    ...
    #     if request.user.profile.id == event.task.owner.id:
    #         if notifications:
    #             for task in data['tasks']:
    #                 for notif in notifications:
    #                     if f'{int(event.alarm[:2]) - 1}:{event.alarm[3:]}' == datetime.datetime.now().strftime('%H:%M'):
    #                         if event.task.id == task.id:
    #                             data['notific'] = notif
    #                         # print(notif.actor)
    #                     else:
    #                         print(datetime.datetime.now().strftime('%H:%M'), ' - ' ,int(event.alarm[:2]) - 1)
    #                         print(notif)
...
def createEvent(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            print(task.id)
            event = Event.objects.get(task=task.id)
            # notify.send(
            #     event, 
            #     recipient=request.user, 
            #     verb=f'Dear {task.owner.user.username} it is time to execute your task({task.name})', 
...
#                     notify.send(eve, recipient=request.user.profile.id, verb='Message', description=request.POST.get('message'))
#             return redirect('index')
#         else:
